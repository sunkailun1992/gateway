...
本规范约束 AI 在 `gateway` 项目中新增或修改注释的方式。项目是 Spring Cloud Gateway 网关，注释必须服务于路由、跨域、请求头透传、Knife4j 聚合、Nacos 配置和安全边界维护。
...
- 配置类、过滤器、路由装配、Knife4j 聚合和 Nacos 读取/发布逻辑应使用类级或方法级 Javadoc 说明职责和边界。
